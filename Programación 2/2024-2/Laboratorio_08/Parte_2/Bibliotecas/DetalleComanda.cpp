...
// Método que permite comparar la proteína del producto.
int DetalleComanda::getProteinaSiEsPlatoFondo() const {
    class PlatoFondo* plato = new class PlatoFondo(*(class PlatoFondo*)this->pedido);
    if (plato != nullptr) {
        return plato->get_proteina();
    }
    return -1;
}


// Sobrecarga del operador "==". Permite comparar si el detalle de la comanda tiene
// un pedido del producto del tipo "Plato Fondo" para poder comparar la proteina del producto.
bool DetalleComanda::operator == (int proteina) {
    if (this->pedido->es_platoFondo()) {
        // El pedido es de producto "Plato Fondo", por lo tanto, comparar la proteína.
        return this->getProteinaSiEsPlatoFondo() == proteina;
    } else {
        return false;
    }
}
