...
// Método que crea el árbol mediante la lectura del archivo "Drones.csv".
...
void Arbol::contarDrones(int & cantSemaforos, int & cantEstacionamiento) {
    conteoDeDrones(this->raiz, cantSemaforos, cantEstacionamiento);
}


void Arbol::conteoDeDrones(class Nodo * arbol, int & cantSemaforos, int & cantEstacionamiento) {
    if (arbol == nullptr) return;
    conteoDeDrones(arbol->izq, cantSemaforos, cantEstacionamiento);
    if (arbol->dron->es_semaforo()) arbol->dron->contar(cantSemaforos);
    if (arbol->dron->es_estacionamiento()) arbol->dron->contar(cantEstacionamiento);
    conteoDeDrones(arbol->der, cantSemaforos, cantEstacionamiento);
}


void Arbol::actualizarDrones(int cantSemaforos, int cantEstacionamiento) {
    actualizacionDeDrones(this->raiz, cantSemaforos, cantEstacionamiento);
}


void Arbol::actualizacionDeDrones(class Nodo * arbol, int & cantSemaforos, int & cantEstacionamiento) {
    if (arbol == nullptr) return;
    actualizacionDeDrones(arbol->izq, cantSemaforos, cantEstacionamiento);
    if (arbol->dron->es_semaforo() and cantSemaforos > 0) {
        arbol->dron->actualizar();
        cantSemaforos--;
    }
    if (arbol->dron->es_estacionamiento() and cantEstacionamiento > 0) {
        arbol->dron->actualizar();
        cantEstacionamiento--;
    }
    actualizacionDeDrones(arbol->der, cantSemaforos, cantEstacionamiento);
}
